    ...
    <div class="bread">
        ...
        <div class="info">
            @foreach($types as $type => $name)
                <span>{{__('hotdesk.support_ticket')}} {{$name}} {{Auth::user()->Locale == 'fa' ? fa_number(App\Models\Questions::where('reply_to' , '!=' , 0)->count()) : App\Models\Questions::where('reply_to' , '!=' , 0)->count()}}</span>
            @endforeach
        </div>
                ...
                @foreach($dataTypeContent as $data)
                    <tr>
                        ...
                        <td class="no-sort no-click" id="bread-actions">
                            ...
                            @if($data->client)
                                <button id="open_chat_naeim" class="openchat1" style="width: 24px;height: 24px;border: 0;background: url({{ asset('assets/images/icons/note.svg') }}) no-repeat center;cursor: pointer;margin-left: 25px;" data-id="{{$data->id}}"
                                    data-name="{{ $data->client->username ?? '' }}" data-clientid="{{$data->client->id}}"
                                    data-items='@json(\App\Models\Questions::query()->where('client_id' , $data->client_id)->orderBy('created_at')->get())'></button>
                            @endif
    ...
    <div class="replaychat" style="max-width: 500px">
        <div class="chat">
            ...
            <div class="chatbox">
                ...
                <div class="messages  mCustomScrollbar" id="items_and_messages" data-mcs-theme="dark">
                </div>
                <div class="sendbox">
