...
$(document).ready(function () {
  $(".form-group select").select2({
    ...
    "language": {
      "noResults": function noResults() {
        if ($('body').hasClass('en')) return "no results";else return "یافت نشد";
      }
    },
  ...
  $('.form-group select').on('select2:opening', function (e) {
    ...
    if ($('body').hasClass('en')) $(this).data('select2').$dropdown.find(':input.select2-search__field').attr('placeholder', 'search');else $(this).data('select2').$dropdown.find(':input.select2-search__field').attr('placeholder', 'جستجو');
