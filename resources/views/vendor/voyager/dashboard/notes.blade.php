<div class="notes @if(!Request::is('admin')) hid @endif">
  ...
  <h3>{{__('hotdesk.notes')}}</h3>
  <div class="textbox">
    <textarea placeholder="{{__('hotdesk.notes_text')}}"></textarea>
    <input type="hidden" name="status" value="red"/>
    <div class="bottom">
      <div class="notestatus">
        <span>{{__('hotdesk.notes_status')}}</span>
        <ul>
  ...
  <div class="notelist">
    <div class="notestatus">
      <span>{{__('hotdesk.notes_list')}}</span>
      <ul>
        <li class="red active" data-id="red"></li>
        <li class="yellow" data-id="yellow"></li>
        <li class="green" data-id="green"></li>
    ...
    <div class="list mCustomScrollbar" data-mcs-theme="dark">
      ...
      <div class="item {{$note->status}}">
        <div class="avatar">
          <img src="{{$user_avatar}}" onerror="this.className='none'" alt="{{$note->user()->exists() ? $user->name : ''}}" title="{{$note->user()->exists() ? $user->name : ''}}" />
        ...
        <div class="notedata">
          ...
          <div class="action">
            @if($note->user_id == Auth::user()->id)
              <button type="button" class="deletenote" data-id="{{$note->id}}"><span>{{__('hotdesk.notes_delete')}}</span></button>
            @endif
          </div>
        </div>
      </div>
