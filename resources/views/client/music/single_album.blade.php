...
@section('content')
                ...
                <div class="space col-lg-12 col-md-12 col-sm-12">
                            ...
                            <div id="white-box" class="white-box  blog-item ">
                                <div class="row padding">
                                    ...
                                    <div class=" padding2 col-lg-6 col-md-9 col-sm-8">
                                        ...
                                        <p class="artist-name">
                                            @foreach($album->artist as $artist)
                                                <a href="{{ route('artist' , $artist->slug) }}"
                                                   style="color: inherit;">
                                                    {{$artist->full_name}}
                                                </a>
                                            @endforeach
                                    ...
                                    @if($has_buy)
                                        <div class="buy-album  col-lg-4 col-md-6 col-md-pull-6 col-sm-12">
                                            ...
                                            <div class="flex-box">
                                                ...
                                                @endif
                                                <a href="{{ $album->file }}" class="submit-filter submit-button">
                                                    دانلود
                                                </a>
                                            </div>
                                        </div>
                                    @elseif($has_membership)
                                        <div class="buy-album  col-lg-4 col-md-6 col-md-pull-6 col-sm-12">
                                            @if($album->discount_price > 0)
                                                <h5>
                                                    {{fa_number(number_format($album->price))}} تومان
                                                </h5>
                                                <h2>
                                                    {{fa_number(number_format($album->discount_price))}} تومان
                                                </h2>
                                            @else
                                                <h2>
                                                    {{fa_number(number_format($album->price))}} تومان
                                            ...
                                            @endif
                                            <a id="buy-payment" class="submit-filter submit-button"
                                               href="{{ route('buy_album_with_member_ship' , $album->id) }}">
                                                <div class="icon-item">
                                                    <img src="{{ asset('client/assets/icon/play2.svg') }}">

                                                </div>
                                                پخش این آلبوم با خرید اشتراک
                                            </a>

                                        </div>
                                    @elseif($album->price > 0)
                                        <div class="buy-album  col-lg-4 col-md-6 col-md-pull-6 col-sm-12">
                                            @if($album->discount_price > 0)
                                                <h5>
                                                    {{fa_number(number_format($album->price))}} تومان
                                        ...
                                        <div class="col-lg-12 col-md-12 col-sm-12">
                                            <div class="music-row">
                                                <div class="music-item flex-box">
                                                    ...
                                                    <div class="flex-box">
                                                        @if(!$album->price == 0 && !$has_buy)
                                                            <img class="key"
                                                                 src="{{asset('client/assets/icon/key2.svg')}}">
                                                        @endif
                                                        <div class="play-music flex-box"
                                                             onclick="(() => { if($('#audio_source').attr('src') !== '{{$music->file}}'){ $('#audio_source').attr('src' , '{{$music->file}}'); $('#audio__')[0].load();} $('#pause_{{ $loop->index }}').toggle('show'); $('#play_{{ $loop->index }}').toggle('show');  togglePlay()})()">
                                                            <img src="{{asset('client/assets/icon/pause.svg')}}"
                                                                 id="pause_{{ $loop->index }}" style="display: none">
                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="white-box  blog-item payment-row">
                                @if(auth()->guard('clients')->check())
                                    <form method="post" action="{{ route('submit_album') }}" id="main_form">
                                        @csrf
                                        <input type="hidden" value="{{ $album->id }}" name="album_id">
                                        <div class="input-row">
                                            <textarea id="text_of_comment" placeholder=" نظر خودتان را بنویسید"
                                ...
                                @endif
                                @each('components.comment' , $album->comments , 'comment')
                            </div>
                    ...
                    <div class="row margin">
                        ...
                        <div class="space col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            <div class="white-box">
                                <div class="row">
                                    @each('components.album' , $most_view , 'album')
                                </div>
                            </div>
                        </div>
    ...
    <div class="audio green-audio-player">
        ...
        @if($album->music && $album->music->count() > 0)
            <audio id="audio__" crossorigin>
                <source id="audio_source" src="{{ $album->music->first()->file }}" type="audio/ogg">
            </audio>
        @else
            <audio id="audio__" crossorigin>
                <source id="audio_source" src="{{ $album->file }}" type="audio/ogg">
            </audio>
        @endif
    </div>
    ...
    <script>
        function submit_form() {
            if ($("#text_of_comment").val().length === 0)
                return;
